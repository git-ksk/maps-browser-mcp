...
| Recent/history synced to account | V5-E evaluated / blocked | Fresh 2026-08-18 observation: Maps History exits to `myactivity.google.com/search-services/history/maps`, requiring a separate account-surface threat model/allowlist review. Maps-local Recent exists, but its current main surface is mutation-adjacent (checkboxes + Delete/More + scrolling) and exposes no stable activity `listitem`/`row` semantics suitable for bounded parsing without DOM-order/private-label dependence. No history/Recent read tool is exposed. |
...
| Saved lists / synced saved places | login required / V5 design-only | Account-backed saved state is staged behind the V5 authentication/isolation entry gate; first candidates are bounded selected-place save membership and existing-list identities, not full Saved-library traversal. |
| Timeline | not a current desktop Maps Web capability | Current Google documentation says Timeline is unavailable in Maps on a computer and device-based. Fresh 2026-08-18 observation found a `/maps/timeline` menu link, but navigation did not remain on a dedicated Timeline route and resolved to the ordinary map surface. Keep it out of the V5 Web candidate set; re-open only if Google restores an official bounded desktop Maps surface. |
