...
| Timeline | not a current desktop Maps Web capability | Current Google documentation says Timeline is unavailable in Maps on a computer and device-based. Remove it from the V5 Web candidate set; re-open only if Google restores an official desktop Maps surface with a bounded semantic target. |
