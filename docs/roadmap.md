...
`main` is now on the **v0.4.0 release train** and contains post-v0.3.3 Handoff/production hardening including first-class managed WebSocket fallback for Cloud Run, bounded operator diagnostics, iPhone Safari keyboard/focus diagnostics, and stopped-profile durability work. These changes are not yet a stable release; they graduate together only after the physical-acceptance and distribution gates below pass.
...
Maps consumes the extracted `mcp-execution-handoff` upstream through an immutable dependency pin. The v0.3.3 baseline includes the physically accepted Handoff-owned Safari WebRTC path for same-LAN direct and cellular TURN relay, while Maps itself remains transport-blind and owns only the Maps-specific Human-intervention lifecycle. The current v0.4.0 release train pins the Handoff **v0.4.4 source release** (`57e4941a5cea9111b5618ce22d5281bd739b49ab`). It includes the prior expiry fencing plus the completed public-WSS correctness and immediate-hardening lines: HEAD-probe preservation, Safari IME settlement, explicit first-frame startup, reconnect frame restoration, higher healthy frame cadence, third-party iOS replacement support, and a deterministic consumer-refresh contract. The two-real-adapter validation with Japan Cinema remains complete. This integration does not generalize the Maps server into a browser/desktop/shell MCP.
...
The roadmap is organized around completed capability baselines and evidence-driven release themes. Version labels are planning targets rather than promises; scope may move when live Google Maps Web behavior, MCP host support, or safety constraints require it.
...
- **Credential-safe Human Handoff baseline** — v0.3.3 physically accepted same-LAN direct and cellular TURN, exact-window fencing, and real Human-only Google sign-in recovery through fresh `signed_in` plus a bounded V5-B read.

### Next — v0.4.0: managed WSS Cloud Run and distribution closeout

v0.4.0 graduates the Cloud Run/Handoff work accumulated after v0.3.3 from “implemented” to “reproducibly accepted in production.” It is not intended to widen the Maps semantic scope.
...
3. **#183 / #135 / #196 post-Human profile lifecycle** — stopped-profile staging and the stable `signed_in` verifier are implemented, and #195 Linux exact-window physical acceptance is complete. Cloud Run physical acceptance still fails closed because fresh Agent verification does not reach stable `signed_in` after a visibly signed-in Human session. Raising the service to 2 vCPU / 4 GiB materially improved takeover responsiveness, but the failure still reproduced after waiting about 10 seconds on the signed-in Maps surface before Done, ruling against resource pressure or an early Done as the sole cause. The next decisive isolation test is to **gracefully close the Human Chrome and launch fresh Agent Chrome against the unchanged local profile while bypassing only the pre-verification local tar->restore round-trip**. Never publish a durable checkpoint before stable verification and never auto-replay the interrupted action. See [Cloud Run profile acceptance handoff](cloud-run-profile-acceptance-handoff.md).
...
### Then — v0.6.0: reliability, UI-change resilience, and observability
...
- Absorb #143 CI-efficiency work in this hardening lane if it can be done without weakening required checks.
...
## Cross-repository Handoff follow-ups — Maps release-blocker boundary
...
- **Resolved upstream baseline:** the closed Handoff lifecycle/reconnect fixes previously tracked by #172/#143/#177/#226, plus the completed v0.4.3/v0.4.4 public-WSS hardening, are included in the pinned v0.4.4 source release. Maps #156 is closed; #181 remains the WSS-only consumer acceptance gate and #183/#135 own the post-Human profile/durability gate. #134 remains open only as non-blocking generic mobile-input UX evidence unless it prevents the real authentication flow.
...
The earlier adaptive-mobile/safe-reconnect and Thin Takeover issues are already closed upstream. The roadmap should not retain closed historical issue numbers as unfinished work; release-blocker decisions use current open issues.
