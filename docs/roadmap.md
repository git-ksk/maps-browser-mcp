...
Maps consumes the extracted `mcp-execution-handoff` upstream through an immutable dependency pin. The v0.3.3 baseline includes the physically accepted Handoff-owned Safari WebRTC path for same-LAN direct and cellular TURN relay, while Maps itself remains transport-blind and owns only the Maps-specific Human-intervention lifecycle. The current v0.4.0 release train pins immutable Handoff commit `0573070061eaec82f96e4fc38e5e4813327d6019`, which is the **v0.4.5 source release** (`523b547`) plus the post-release consumer fix #263. This carries the v0.4.5 external Human-surface authority, WSS terminal-intent, and inbound-queue hardening, then adds one content-free shared status card so WebRTC and managed WSS make pre-first-frame, reconnect, unavailable, and verifying states visible to the Human. Authority, transport selection, Human input, credentials, and semantic verification are unchanged. The two-real-adapter validation with Japan Cinema remains complete. This integration does not generalize the Maps server into a browser/desktop/shell MCP.
