...
V5 does not require this ceremony when the dedicated profile is already signed in. A persistent, dedicated, single-user profile may be signed in locally by the Human and then reused; in that case fresh `maps_read_authenticated_readiness` can return `signed_in` and the bounded V5 tools proceed without any remote takeover transport. The handoff mechanisms below exist for later Human-only sign-in/re-authentication, consent, or challenge handling.

When `MAPS_CREDENTIAL_SAFE_HANDOFF=true` is enabled, `maps_request_human_sign_in` provides an explicit Human-only entry path from `signed_out`. The tool never clicks Sign in, selects a Google Account, enters credentials/MFA on behalf of the Human, reads account identity, or exports cookies/tokens. Form-elicitation-capable MCP clients keep the signed requestState / `input_required` MRTR flow. Clients that do not advertise form elicitation use a bounded explicit fallback: the request tool returns only the authenticated short-lived takeover locator, `maps_complete_human_sign_in` performs revoke → Human Chrome stop/quiescence → unpublished candidate staging → fresh stable verification against the unchanged local profile → verified candidate promotion, and `maps_cancel_human_sign_in` revokes without staging or promoting a signed-in profile. No intervention id, principal binding, capability, credential, cookie, or account identity is returned by that fallback.

The default `MAPS_CREDENTIAL_SAFE_TRANSPORT=external` remains a backward-compatible fail-closed choice. `cua_takeover`, `thin_takeover`, and `webrtc_takeover` are implementation siblings, but they all obey the same browser-lifecycle invariant: stop the Agent automation browser and open the same dedicated profile in a normal browser with no remote-debugging/automation authority. `webrtc_takeover` is physically accepted on macOS/iPhone Safari over direct and TURN. Linux/container implements the same normal-browser boundary with a Handoff-owned Linux capture/input host; Ubuntu/Xvfb acceptance is green, and Cloud Run has passed the core production transport/sign-in path through Cloudflare Realtime TURN, physical iPhone Safari `Live · relay`, and real Human-only Google sign-in. That Cloud Run evidence does not overclaim the still-open physical iPhone session-stability gate (#156) or explicit Done -> revoke -> fresh `signed_in` -> stopped-profile checkpoint -> fresh restore lifecycle (#135). Mobile keyboard/CJK polish (#134) is non-blocking unless it prevents the real authentication flow; ordinary Maps interaction remains MCP-owned. The legacy `hosted_cdp` value, which kept managed Chromium alive for Human-owned CDP, fails closed for sign-in/consent/access-challenge control after physical Cloud Run acceptance showed it is not equivalent to a normal browser. After the normal browser is opened, Maps issues only a short-lived Handoff locator that Safari can open directly. Maps passes only the PID of the normal Chrome process it starts; Handoff resolves exactly one eligible window for that PID and scopes capture/input to that window, failing closed rather than exposing the desktop when the target is missing or ambiguous. ScreenCaptureKit/VideoToolbox, WebRTC signaling/RTP/DataChannel, direct touch/keyboard input, generation fencing, reconnect, and immediate revoke stay inside `mcp-execution-handoff`; Maps does not own those implementation details. WebRTC-only locators cannot downgrade to the legacy HTTP frame/input UI. Human-entered text remains on the Human plane and is never returned through northbound MCP/model/logging. The macOS end-to-end V5 sign-in acceptance remains the complete Done/revoke reference path; Cloud Run's post-Done durability evidence remains tracked separately in #135. Native-app physical acceptance remains separately tracked upstream and does not block the WebRTC path.

Human completion is not proof that a particular Google Account is active. The normal browser and active Human transport are revoked before automation resumes, the dedicated profile must be released, the CDP runtime is relaunched fresh, and the client must call `maps_read_authenticated_readiness` again. Pre-auth semantic state is never replayed. No third-party remote-desktop product is required by `cua_takeover`; `external` remains available for deployments that prefer one.
