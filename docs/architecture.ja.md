...
通常Maps automationはprocess-owned Chrome + CDPを維持します。**credential-safe Human controlのbrowser lifecycleはOS/transport共通の不変条件**です。Agent CDP authorityをdetachしautomation browser processを停止し、same dedicated profileをremote-debugging / automation authorityなしのnormal browserで開きます。Human transportはOSごとに異なっても、そのnormal-browser surfaceだけを操作対象にします。Human完了/revoke後はHuman surfaceを閉じてからfresh Agent browser/CDPでreadinessを再検証します。invariantは **Humanはnormal browser + exclusive authority + bounded Human control + fresh post-Human verification** です。managed Chromiumを生かすlegacy `hosted_cdp` shapeはcredential / consent / challenge経路として不採用のままfail closedし、Linux remote Human controlはnormal-browser WebRTC hostを使います。
...
Agent-owned automation CDP Chrome
  -> Human authority
  -> supported OSすべてでautomation Chromeを停止
  -> same dedicated profileをremote-debugging / automation authorityなしのnormal browserで再open
  -> Human transportはそのexact normal-browser surfaceだけを操作
       macOS accepted: browser `webrtc_takeover`; external / Cua / Nativeはoptional sibling
       Linux/container: remote Human sign-in有効化前にLinux normal-browser capture/input hostが必要
  -> Human transport/leaseをrevokeしnormal browserを終了
  -> fresh Agent CDP attachment
  -> fresh readiness / semantic validation
  -> optional verified stopped-profile deployment checkpoint
```

`thin_takeover` ではNative ScreenCaptureKit / VideoToolbox / CoreGraphics data planeと短命Native locatorをHandoffが所有します。`webrtc_takeover` は、Mapsが `TakeoverBroker` / `SpawnedWebRtcRuntimeProvider` / `createWebRtcLink()` を自前compositionする方式から、Handoff first-class `BrowserHandoffAdapter` を利用する方式へ移行しました。Mapsが渡すのはintervention / principal binding、自分が直前に起動したnormal ChromeのPID、明示的なcredential-safe Human input policyだけです。WebRTC runtime / signaling、H.264 / RTP video、direct-touch / keyboard DataChannel、direct-first / TURN transport、generation-bound reconnect、disconnect / reload後のcompletion-only recovery、concrete route ownership、browser transport teardownはHandoffが所有します。HandoffはPIDからeligibleなon-screen windowを厳密に1つ解決（または明示windowを再検証）し、そのsurfaceだけへcapture/inputを限定し、LinuxではHuman mutation直前にX11 ownership / focusも再検証してmissing / ambiguous / stale targetをfail closedします。Mapsはplatform window探索を所有せず、SDP / ICE / RTP / framebuffer bytes / raw Human inputも扱いません。WebRTC-only locatorから旧HTTP frame/input UIへのfallbackも不可です。

Current deploymentでは各transport実装をsiblingとして扱います。`webrtc_takeover` はmacOSでsame-LAN direct / cellular TURN / real Google sign-in recoveryまで物理acceptance済みです。Linux siblingも同じ **normal-browser authority boundary** を実装し、Handoff Ubuntu/Xvfb exact-window media/input acceptanceに加え、Cloud Run productionでCloudflare Realtime TURN、物理iPhone Safari `Live · relay`、real Human-only Google sign-inまで物理acceptance済みです。Physical iPhone sign-in session-stabilityは#156、より狭いpost-Done revoke / checkpoint / fresh-restore lifecycleは#135をgateとして追跡します。Mobile keyboard / CJK polishの#134は、実際の認証フローを止めない限り非blockerのgeneric Handoff follow-upです。`thin_takeover` はoptional / experimentalです。以前の `hosted_cdp` experimentはCloud Run実機Google sign-in acceptanceで失敗したため、credential / consent / challenge Human controlではfail closedするcompatibility valueとしてのみ残します。defaultは `external` です。
