...
Current deploymentでは各transport実装をsiblingとして扱います。`webrtc_takeover` はmacOSでsame-LAN direct / cellular TURN / real Google sign-in recoveryまで物理acceptance済みです。Linux siblingも同じ **normal-browser authority boundary** を実装し、Handoff Ubuntu/Xvfb acceptanceでexact-window capture/input、H.264/WebRTC、focused text input、teardownまでPASSしています。ただしCloud Run + 物理iPhoneでのreal Google sign-in acceptanceは未実施です。`thin_takeover` はoptional / experimentalです。以前の `hosted_cdp` experimentはCloud Run実機Google sign-in acceptanceで失敗したため、credential / consent / challenge Human controlではfail closedするcompatibility valueとしてのみ残します。defaultは `external` です。
