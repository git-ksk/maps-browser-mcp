...
専用profileがすでにsign-in済みなら、V5利用にこのceremonyは不要です。Humanがローカルでログインしたpersistent / dedicated / single-user profileをそのまま利用でき、freshな `maps_read_authenticated_readiness` が `signed_in` ならremote takeover transportなしでbounded V5 toolへ進めます。以下のHandoff機構は、後からHuman-onlyのsign-in / re-authentication、consent、challenge対応が必要になった場合のためにあります。

`MAPS_CREDENTIAL_SAFE_HANDOFF=true` の場合、`maps_request_human_sign_in` が `signed_out` から明示的なHuman-only entry pathを提供します。Sign in click、Google Account選択、Humanに代わるcredential/MFA入力、account identity読取、cookie/token exportは行いません。form elicitation対応MCP clientではsigned requestState / `input_required` のMRTRを維持します。form elicitationをadvertiseしないclientではboundedなexplicit fallbackを使い、request toolはauthenticated / short-lived takeover locatorだけを返し、`maps_complete_human_sign_in` がrevoke → Human Chrome停止/quiescence → 未公開candidate stage → unchanged local profileでfresh Agent stable verification → verified candidate promoteを実行し、`maps_cancel_human_sign_in` はcandidate stage / promoteなしでrevokeします。このfallbackからintervention id、principal binding、capability、credential、cookie、account identityは返しません。

defaultの `MAPS_CREDENTIAL_SAFE_TRANSPORT=external` はbackward-compatible / fail-closed choiceとして維持します。`cua_takeover` / `thin_takeover` / `webrtc_takeover` は実装siblingですが、browser lifecycleの不変条件は共通です。Agent automation browserを停止し、same dedicated profileをremote-debugging / automation authorityなしのnormal browserで開きます。`webrtc_takeover` はmacOS + iPhone Safariでdirect / TURNとも物理acceptance済みです。Linux / containerも同じnormal-browser boundaryをHandoff-owned Linux capture/input hostで実装済みで、Ubuntu/Xvfb acceptanceはgreen、Cloud RunもCloudflare Realtime TURN、物理iPhone Safari `Live · relay`、real Human-only Google sign-inまでcore production transport / sign-in acceptance済みです。ただしCloud Runで実際のsign-inを最後まで安定して通すphysical iPhone session gateは#156、明示Done -> revoke -> fresh `signed_in` -> stopped-profile checkpoint -> fresh restoreまでを通すdurability lifecycleは#135としてv0.4で追跡します。Mobile keyboard / CJK UXの#134は、実際の認証フローを止めない限り非blockerです。managed Chromiumを生かしてHuman-owned CDPで操作していたlegacy `hosted_cdp` は、Cloud Run物理acceptanceでnormal browserと同等でないことが判明したためsign-in / consent / access-challenge controlではfail closedします。normal browser開始後、Mapsが発行するのはshort-lived Handoff locatorだけです。MapsがHandoffへ渡すcapture ownership情報は、自分が起動したnormal ChromeのPIDだけです。HandoffはそのPIDからeligibleなwindowを厳密に1つ解決し、そのwindowだけへcapture/inputをscopeし、missing/ambiguousならdesktopへ広げずfail closedします。ScreenCaptureKit / VideoToolbox、WebRTC signaling / RTP / DataChannel、direct touch / keyboard input、generation fencing、reconnect、immediate revokeは `mcp-execution-handoff` 内に留め、Mapsは実装詳細を所有しません。Human入力textはHuman plane内だけを通りnorthbound MCP/model/logへ返しません。macOSのend-to-end V5 sign-in acceptanceはDone/revokeまで含むcomplete reference pathのまま維持し、Cloud Run post-Done durability evidenceは#135で別追跡します。Native appの物理acceptanceはupstreamで別追跡し、WebRTC pathをblockしません。
