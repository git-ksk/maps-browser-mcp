...
Transportは5種類です。defaultの `MAPS_CREDENTIAL_SAFE_TRANSPORT=external` は既存deployment向けのbackward-compatible / fail-closed choiceとして維持し、既存OS-level remote-access surfaceへHumanを案内します。`cua_takeover` はlocal fallback/reference bridgeです。`thin_takeover` はoptional / experimentalなNative credential pathです。`webrtc_takeover` はcurrent macOS single-user deployment向けのrecommended built-in mobile pathで、物理iPhone Safariにてsame-LAN direct WebRTCとcellular TURN relayの両方をacceptance済みです。`hosted_cdp` はLinux / Cloud Run向けで、managed Chromium processを生かしたままAgent CDP authorityをfenceし、authenticated bounded brokerからHuman用frame/input surfaceだけを公開します。automation Chrome停止後にsame dedicated profileをnormal Chromeで開き、Mapsはshort-lived Handoff locatorだけを発行します。MapsがHandoffへ渡すcapture ownership情報は、自分が起動したnormal ChromeのPIDだけです。HandoffはそのPIDからeligibleなwindowを厳密に1つ解決し、そのwindowだけへcapture/inputをscopeし、missing/ambiguousならdesktopへ広げずfail closedします。ScreenCaptureKit / VideoToolbox、WebRTC signaling / RTP / DataChannel、direct touch / keyboard input、generation fencing、reconnect、immediate revokeは `mcp-execution-handoff` 内に留め、Mapsは実装詳細を所有しません。Human入力textはHuman plane内だけを通りnorthbound MCP/model/logへ返しません。V5 end-to-end sign-inもfresh `signed_out` → Human-only Google sign-in → revoke / stale-locator fencing → fresh `signed_in` readiness → bounded authenticated V5-B readまで実機acceptance済みです。Native appの物理acceptanceはupstreamで別追跡し、WebRTC pathをblockしません。
