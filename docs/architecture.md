...
Current deployment status keeps transport implementations as siblings. `webrtc_takeover` is physically accepted on macOS (same-LAN direct, cellular TURN relay, and real Google sign-in recovery). The Linux sibling now implements the same **normal-browser** authority boundary and has passed Handoff Ubuntu/Xvfb acceptance for exact-window capture/input, H.264/WebRTC, focused text input, and teardown; Cloud Run + physical-iPhone real Google sign-in acceptance remains pending. `thin_takeover` remains optional/experimental. The earlier `hosted_cdp` experiment failed physical Cloud Run Google sign-in acceptance and is retained only as a compatibility value that fails closed for credential/consent/challenge Human control. The default remains `external`.
