...
Normal Maps automation remains process-owned Chrome + CDP. **Credential-safe Human control has one cross-platform browser-lifecycle invariant:** Agent CDP authority is detached, the automation browser process is stopped, and the same dedicated profile is reopened in a normal browser without remote-debugging or automation authority. Human transport may differ by OS, but it must control only that normal-browser surface. Human completion/revocation closes the Human surface before a fresh Agent browser/CDP attachment verifies readiness. The invariant is **normal browser for Human control, exclusive authority, bounded Human control, and fresh post-Human verification**. The legacy Linux/container `hosted_cdp` shape that kept managed Chromium alive is not an accepted credential/consent/challenge path and remains fail-closed; Linux remote Human control now uses the normal-browser WebRTC host instead.
...
Agent-owned automation CDP Chrome
  -> Human authority
  -> stop automation Chrome on every supported OS
  -> reopen the same dedicated profile in a normal browser with no remote-debugging/automation authority
  -> Human transport controls only that exact normal-browser surface
       macOS accepted: browser `webrtc_takeover`; external/Cua/Native remain optional siblings
       Linux/container: requires a Linux normal-browser capture/input host before remote Human sign-in is enabled
  -> revoke Human transport/lease and close the normal browser
  -> fresh Agent CDP attachment
  -> fresh readiness / semantic validation
  -> optional verified stopped-profile deployment checkpoint
```

For `thin_takeover`, Handoff owns the Native ScreenCaptureKit/VideoToolbox/CoreGraphics data plane and short-lived Native locator. For `webrtc_takeover`, Maps now consumes Handoff's first-class `BrowserHandoffAdapter` rather than composing `TakeoverBroker`, `SpawnedWebRtcRuntimeProvider`, or `createWebRtcLink()` itself. Maps supplies only the intervention/principal binding, the PID of the normal Chrome process it just started, and an explicit credential-safe Human input policy. Handoff owns WebRTC runtime/signaling, H.264/RTP video, direct-touch/keyboard DataChannels, direct-first/TURN transport, generation-bound reconnect, completion-only disconnect/reload recovery, concrete route ownership, and browser transport teardown. Handoff resolves exactly one eligible on-screen window for the PID (or revalidates an explicit window), confines capture/input to that surface, rechecks Linux X11 ownership/focus before Human mutation, and fails closed on missing/ambiguous/stale targets. Maps never owns platform window discovery and never processes SDP, ICE, RTP, framebuffer bytes, or raw Human input. WebRTC-only locators cannot fall back to the legacy HTTP frame/input UI.

Current deployment status keeps transport implementations as siblings. `webrtc_takeover` is physically accepted on macOS (same-LAN direct, cellular TURN relay, and real Google sign-in recovery). The Linux sibling implements the same **normal-browser** authority boundary, passes Handoff Ubuntu/Xvfb exact-window media/input acceptance, and has passed Cloud Run production physical acceptance through Cloudflare Realtime TURN, iPhone Safari `Live · relay`, and real Human-only Google sign-in. The physical iPhone sign-in session-stability gate remains #156 and the narrower post-Done revoke/checkpoint/fresh-restore lifecycle remains #135. Mobile keyboard/CJK polish in #134 is a non-blocking generic Handoff follow-up unless it prevents the real authentication flow. `thin_takeover` remains optional/experimental. The earlier `hosted_cdp` experiment failed physical Cloud Run Google sign-in acceptance and is retained only as a compatibility value that fails closed for credential/consent/challenge Human control. The default remains `external`.
