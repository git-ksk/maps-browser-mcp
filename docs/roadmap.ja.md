...
Mapsは抽出済み `mcp-execution-handoff` upstreamをimmutable dependency pinでconsumerとして組み込みます。v0.3.3 baselineでは、same-LAN direct / cellular TURN relayのHandoff-owned Safari WebRTC pathまで物理acceptance済みですが、Maps自身はtransport-blindのままMaps-specificなHuman-intervention lifecycleだけを所有します。現在のv0.4.0 release trainではHandoff **v0.4.5 source release** (`523b547`) に続くpost-release consumer fix #263まで含むimmutable commit `0573070061eaec82f96e4fc38e5e4813327d6019` をpinします。v0.4.5のexternal Human-surface authority / WSS terminal-intent / inbound-queue hardeningに加え、#263でWebRTC / managed WSSのfirst-frame前・reconnect・unavailable・verifying状態をHumanへ明示するcontent-free status cardを共通化します。Authority、transport selection、Human input、credential、semantic verificationは変更しません。Japan Cinemaを含むtwo-real-adapter validationも完了済みで、この統合でMaps serverをgeneric browser / desktop / shell MCPへ広げません。
