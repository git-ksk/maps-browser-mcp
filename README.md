...
Operationally, `webrtc_takeover` is the physically accepted built-in mobile path today. The **normal-browser boundary is transport- and OS-independent**: Human credential/consent/challenge control must never reuse the Agent automation browser or Human-owned CDP. macOS implements the boundary with normal Chrome plus the Handoff WebRTC host; Linux/container deployments must implement the same boundary with a Linux normal-browser capture/input host before remote Human sign-in is enabled. Legacy `hosted_cdp` therefore fails closed for those Human steps. `external` remains the configuration default and `thin_takeover` remains optional/experimental.
