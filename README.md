...
When Google sign-in, re-authentication, consent, or an access challenge requires a person, `maps-browser-mcp` keeps Maps semantics, profile ownership, and fresh post-Human verification on the Maps side while [`mcp-execution-handoff`](https://github.com/git-ksk/mcp-execution-handoff) owns temporary Human authority, exact-window capture/input, direct WebRTC -> WebSocket relay -> optional TURN fallback, generation fencing, and revoke. Human **Done** ends Human authority; it is not authentication proof or approval for a later Maps mutation. See **[Maps ↔ Handoff responsibility boundary](docs/handoff-overview.md)**.
...
| `MAPS_CREDENTIAL_SAFE_TRANSPORT` | `external` | `external` uses an OS-level local surface; `cua_takeover` uses local Cua; `thin_takeover` uses the Native Thin Takeover runtime; `webrtc_takeover` selects Handoff's install-free Safari surface, whose transport policy is Handoff-owned direct WebRTC -> WebSocket relay -> optional TURN; legacy `hosted_cdp` remains fail-closed for Human credential/consent/challenge control |
| `MAPS_HANDOFF_TRANSPORT_ORDER` | unset | Exact Handoff transport order for `webrtc_takeover`, as unique comma-separated `webrtc_direct`, `websocket_relay`, and/or `webrtc_relay`; one item such as `websocket_relay` is WSS-only, while omission preserves Handoff defaults |
...
| `MAPS_WEBRTC_TAKEOVER_HOST_EXECUTABLE` | unset | Absolute path to the Handoff WebRTC host executable; macOS uses `takeover-webrtc-host`, Linux uses the pinned Linux normal-browser helper/wrapper |
| `MAPS_WEBRTC_TAKEOVER_DISPLAY_ID` | unset | Optional explicit macOS display ID for WebRTC capture |
| `MAPS_WEBRTC_TAKEOVER_DISPLAY_NAME` | unset | Required on Linux `webrtc_takeover`; local X11 display such as `:99` |
...
| [Managed Human Takeover](docs/webrtc-human-takeover.md) | macOS/Linux + iPhone Safari setup, authenticated operator origin, direct/WSS/optional TURN behavior |
