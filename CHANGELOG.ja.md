...
- Issue #74向けにisolatedな `reference/oauth-gateway` dogfood実装を追加。MCP 2026-07-28 shapeのOAuth/CIMD + PKCE、`maps:use` resource scope、`offline_access` refresh、reference専用OAuth state、public OAuth credentialを除去してcurrent-main coreへ送るloopback `static-bearer` private hopを実装。Remote `/takeover/*` はbrowser-session auth boundaryを別途設計するまで意図的に無効のまま。
