...
運用上、現在物理acceptance済みのbuilt-in mobile pathは `webrtc_takeover` です。ただし **normal-browser boundaryはtransport / OS共通の不変条件**です。Humanのcredential / consent / challenge操作でAgent automation browserを再利用したり、Human-owned CDPで操作したりしてはいけません。macOSはnormal Chrome + Handoff WebRTC hostでこの境界を実装済みで、Linux/containerもremote Human sign-inを有効化する前にLinux用normal-browser capture/input hostで同じ境界を実装する必要があります。そのためlegacy `hosted_cdp` はこれらHuman stepでfail closedします。`external` はdefault、`thin_takeover` はoptional / experimentalのままです。
