...
- Advance the pinned `mcp-execution-handoff` integration to the v0.4.4 source release (`57e4941a5cea9111b5618ce22d5281bd739b49ab`). This carries forward the v0.4.2 expiry fencing and adds the v0.4.3/v0.4.4 managed-WSS hardening line: takeover HEAD-probe preservation, Safari IME commit settling, explicit first-frame startup, static-frame reconnect recovery, higher healthy frame cadence, third-party iOS keyboard replacement support, and deterministic consumer-refresh verification. Physical Maps acceptance remains consumer-owned.
...
- Add V5-C bounded existing-list Save: `maps_save_place_to_list` revalidates signed-in readiness, exact place identity, resource epoch, and list index + expected label from a fresh chooser; already-saved is a no-toggle idempotent success, and a mutation succeeds only after exact `aria-checked=true` postcondition verification. New-list creation, unsave/remove, bulk save, Saved-library traversal, and account/credential exposure remain out of scope. Fresh live UI compatibility passed; the real mutation dogfood was intentionally blocked because no uniquely identifiable existing test-purpose list was available.
...
- Start the V5-A foundation gate: add a disabled-by-default `MAPS_V5_AUTHENTICATED_WORKFLOWS` opt-in that requires Interactive Assist, a server-owned absolute dedicated profile, and the current single-user principal shapes; reject external CDP and module auth until per-principal profile isolation exists. Authenticated semantic tools are exposed only when this V5 opt-in is enabled.
