...
export class TakeoverSessionManager {
  ...
  ensure(interventionId: string, epoch: number): TakeoverGrant {
    this.pruneExpired();
    for (const record of this.records.values()) {
      if (!record.revoked && record.interventionId === interventionId && record.epoch === epoch) {
  ...
  private capabilityFor(record: TakeoverRecord): string {
    return createHmac("sha256", this.signingKey)
      .update("maps-browser-mcp/takeover/v1\0")
      .update(record.id)
      .update("\0")
